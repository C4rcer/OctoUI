--[[
	Records which of the names this compatibility layer is about to define
	already exist on the client, BEFORE any of them are overwritten.

	This file and wowAPI.lua assign unconditionally. That is fine for Lua base
	library functions 5.0 genuinely lacks, but not for anything the server's own
	UI provides: OctoWoW ships a tolerant `wipe`, and clobbering it with a strict
	one broke Turtle_ShopUI's search. Anything listed here is a candidate for the
	same problem, so it is captured rather than guessed at.

	Read it in game with /octoui-missing, which copies it into ElvDB.
]]
OctoUI_ClobberedGlobals = {}
do
	local names = {
		--luaAPI.lua
		"getprinthandler", "gmatch", "print", "select", "setprinthandler",
		"strconcat", "strjoin", "strmatch", "strrev", "strsplit", "strtrim",
		"tostringall", "wipe",
		--wowAPI.lua
		"BetterDate", "CreateStatusBarTexturePointer", "EditBoxGetCursorPosition",
		"EditBoxSetCursorPosition", "FillLocalizedClassList", "GetAverageItemLevel",
		"GetContainerNumFreeSlots", "GetCurrentMapAreaID", "GetInstanceInfo",
		"GetInventoryItemDurability", "GetItemCount", "GetItemFamily",
		"GetItemInfoByName", "GetItemLevelColor", "GetMapNameByID",
		"GetPlayerFacing", "GetQuestDifficultyColor", "GetQuestItemStarterInfo",
		"GetShapeshiftForm", "GetThreatStatus", "GetThreatStatusColor",
		"HookScript", "ToggleFrame", "UnitAura", "difftime", "hooksecurefunc",
		"issecure", "issecurevariable", "securecall", "tContains"
	}

	local n = 0
	for i = 1, table.getn(names) do
		if type(getglobal(names[i])) == "function" then
			n = n + 1
			OctoUI_ClobberedGlobals[n] = names[i]
		end
	end
end
...
--This client ships its own tolerant `wipe`, and its FrameXML depends on it:
--Turtle_ShopUI's ShopFrame_Search calls wipe() with no argument, which is
--harmless against the client version but threw against the strict one below,
--breaking the in-game shop search. Do not clobber a wipe that already exists.
--
--More generally, the polyfills in this file overwrite unconditionally. That is
--safe for Lua base-library functions 5.0 genuinely lacks, but not for anything
--the server's own UI might also provide.
if type(wipe) ~= "function" then
	...
	wipe = table.wipe
elseif type(table.wipe) ~= "function" then
	--Global exists but the table.* alias does not; point it at the client's
	table.wipe = wipe
end
