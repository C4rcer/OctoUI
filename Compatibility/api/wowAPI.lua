...
--This client already provides tContains. Same semantics, but there is no reason
--to overwrite a client function; see the wipe note in luaAPI.lua for why that
--matters here.
if type(tContains) ~= "function" then
...
end --if type(tContains) ~= "function"
...
-- Custom server instances (OctoWoW / Turtle). LibBabble-Zone has no entries
-- for these, so key them by the literal name GetRealZoneText() reports. Add
-- entries as they are confirmed in-game; see /octoui-zonedump.
local customZoneInfo = {
	-- ["Example Raid"] = {mapID = 9001, maxPlayers = 20},
}

for mapName in pairs(customZoneInfo) do
	zoneInfo[mapName] = customZoneInfo[mapName]
end
...
local function GetMaxPlayersByType(instanceType, zoneName)
	...
	local info = zoneName ~= "" and zoneInfo[zoneName]
	if info then
		return info.maxPlayers
	end

	-- Unlisted instance, i.e. custom server content. Never fall through to nil:
	-- GetInstanceInfo passes this into format("%d"), which errors on nil.
