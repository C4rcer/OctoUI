...
L["Hotkey Text X-Offset"] = "快捷键文字X偏移"
L["Hotkey Text Y-Offset"] = "快捷键文字Y偏移"
L["Hours"] = "时"
...
范例：Interface\AddOns\OctoUI\media\textures\copy
...
L["The unit prefixes you want to use when values are shortened in ElvUI. This is mostly used on UnitFrames."] = "在OctoUI中数值单位的缩写. 该选项主要应用在单位框体"
L["Thin Border Theme"] = "细边框主题"
L["This is for Customized Icons in your Interface/Icons folder."] = "这将应用于你的Interface/Icons文件夹下的自定义图标"
L["This selects the Chat Frame to use as the output of ElvUI messages."] = "选择OctoUI信息的聊天输出"
...
--Media
L["Applies the font and font size settings throughout the entire user interface. Note: Some font size settings will be skipped due to them having a smaller font size by default."] = "把该字体设置应用到所有OctoUI设置中去,但是某些设置并不会被改变"
...
L["Replaces the default Blizzard fonts on various panels and frames with the fonts chosen in the Media section of the ElvUI config. NOTE: Any font that inherits from the fonts ElvUI usually replaces will be affected as well if you disable this. Enabled by default."] = "用OctoUI字体设置代替暴雪原有字体设置,如果禁用有可能导致你的UI出问题,默认开启开选项"
...
L["Use Default"] = "使用默认值"
L["Use Health Texture Backdrop"] = "对背景使用生命条材质"
