...
function AB:PositionAndSizeBarShapeShift()
	...
	--Classes with no forms (mage, warlock, hunter, shaman, and warriors below
	--level 10) still had the bar drawn: every button hides, but bar.LastButton
	--is never set, so the sizing below fell back to the full NUM_SHAPESHIFT_SLOTS
	--and left an empty backdrop parked in the corner. Hide the whole bar instead,
	--UPDATE_SHAPESHIFT_FORMS brings it back the moment a form is learned.
