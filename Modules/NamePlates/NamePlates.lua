...
function mod:OnShow(self, isUpdate)
	...
	if mod.db.units[unitType].healthbar.enable then
		...
		--otherwise a plate reused for another unit type keeps the previous settings
...
function mod:Initialize()
	...
	--SuperWoW hands out the unit GUID on the plate's parent frame, which is the only
	--way to read auras off a plate that is neither the target nor the mouseover
