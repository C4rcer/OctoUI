...
function mod:StyleFilterConfigureEvents()
	...
	self.StyleFilterCheckAuras = nil
	...
	for filterName, filter in pairs(E.global.nameplates.filters) do
		if filter.triggers and E.db.nameplates and E.db.nameplates.filters then
			if E.db.nameplates.filters[filterName] and E.db.nameplates.filters[filterName].triggers and E.db.nameplates.filters[filterName].triggers.enable then
				...
				-- aura names are only resolved (tooltip scan) when something asks for them
				if (filter.triggers.buffs and filter.triggers.buffs.names and next(filter.triggers.buffs.names))
				or (filter.triggers.debuffs and filter.triggers.debuffs.names and next(filter.triggers.debuffs.names)) then
					self.StyleFilterCheckAuras = true
				end
