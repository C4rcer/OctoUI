...
--AceTimer for the minimap button sweep: addons add their buttons at different
--moments, so MinimapButtons.lua re-scans a few times after login.
local M = E:NewModule("Minimap", "AceEvent-3.0", "AceTimer-3.0");
E.Minimap = M
...
local menuList = {
	...
	--HELPFRAME_HOME_ISSUE3_HEADER is absent on this client, which left this entry
	--with no label. PVP carries the same string here.
	{text = PVP or HELPFRAME_HOME_ISSUE3_HEADER or "PvP",
...
function M:Initialize()
	...
	MiniMapMailIcon:SetTexture("Interface\\AddOns\\OctoUI\\media\\textures\\mail")

	MiniMapBattlefieldBorder:Hide()

	E:CreateMover(MMHolder, "MinimapMover", MINIMAP_LABEL, nil, nil, MinimapPostDrag)

	--After MMHolder exists, since the button bar anchors under it by default
	self:LoadMinimapButtonBar()
	...
	--Stays a real "Minimap": farm mode shows THIS frame enlarged and hides the
	--normal one, so it has to render a map. Only MinimapCluster (zone text,
	--tracking button) is moved on top of it by the OnShow below.
	--It cannot host a child frame on this client, which is why E:CreateBackdrop
	--below used to die with "CreateFrame: Couldn't find 'this' in parent
	--object". That is now handled in the toolkit: the backdrop gets hosted on
	--this frame's parent and anchored here, so it looks identical.
