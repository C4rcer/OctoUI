...
local function LoadSkin()
	...
	for i = 1, getn(texs) do
		local scriptTex = _G["ScriptErrorsFrame"..texs[i]]
		local traceTex = _G["EventTraceFrame"..texs[i]]
		if scriptTex and scriptTex.SetTexture then scriptTex:SetTexture(nil) end
		if traceTex and traceTex.SetTexture then traceTex:SetTexture(nil) end
	end
