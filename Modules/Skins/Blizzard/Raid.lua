	...
	local function skinPulloutFrames()
		for i = 1, NUM_RAID_PULLOUT_FRAMES do
			...
			if not rp.backdrop then
				local skinObj = _G["RaidPullout"..i.."MenuBackdrop"]
				if skinObj then skinObj:SetBackdrop(nil) end
		...
		for i = 1, pullOutFrame.numPulloutButtons do
			...
			if not pfBObj.backdrop then
				for _, v in pairs{"HealthBar", "ManaBar", "Target", "TargetTarget"} do
					...
					local skinObj = _G[sBar]
					if skinObj then skinObj:SetStatusBarTexture(E.media.normTex) end
				end
