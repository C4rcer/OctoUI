...
local function LoadSkin()
	...
	for _, editbox in pairs(editboxs) do
		...
		local skinObj = _G[editbox]
		if skinObj then skinObj:SetTextInsets(1, 1, -1, 1) end
	end
	...
	--These two checkboxes are not present on every client build
	if IsUsableCheckButton then
	...
	if ShowOnPlayerCheckButton then
