...
local function LoadSkin()
	...
	local function HandleResistanceFrame(frameName)
		for i = 1, 5 do
			...
			--Reuse the lookup above, and do not assume both regions came back
			local icon, text
			if frame then icon, text = frame:GetRegions() end

			if icon then
				E:SetInside(icon)
				icon:SetDrawLayer("ARTWORK")
			end
			if text then text:SetDrawLayer("OVERLAY") end
	...
	for i = 1, NUM_FACTIONS_DISPLAYED do
		...
		factionHeader:SetNormalTexture("Interface\\AddOns\\OctoUI\\media\\textures\\PlusMinusButton")
	...
	SkillFrameCollapseAllButton:SetNormalTexture("Interface\\AddOns\\OctoUI\\media\\textures\\PlusMinusButton")
	...
	for i = 1, SKILLS_TO_DISPLAY do
		...
		label:SetNormalTexture("Interface\\AddOns\\OctoUI\\media\\textures\\PlusMinusButton")
