...
function S:HandleButton(f, strip, isDeclineButton)
	...
	if isDeclineButton then
		...
		if not f.text then
			...
			f.text:SetFont([[Interface\AddOns\OctoUI\media\fonts\PT_Sans_Narrow.ttf]], 16, "OUTLINE")
			f.text:SetText("x")
...
function S:HandleNextPrevButton(btn, useVertical, inverseDirection)
	...
	if not btn.icon then
		...
		btn.icon:SetTexture([[Interface\AddOns\OctoUI\media\textures\SquareButtonTextures]])
...
function S:HandleEditBox(frame)
	...
	--Type-checked rather than a plain nil check: on this client an EditBox that
	--never got a backdrop reports frame.backdrop as a function, so "if
	--frame.backdrop then" passes and the index below still dies.
	if type(frame.backdrop) == "table" then
	...
	if frame:GetName() then
		...
		if type(frame.backdrop) == "table"
		and (gfind(frame:GetName(), "Silver") or gfind(frame:GetName(), "Copper")) then
...
function S:HandleDropDownBox(frame, width)
	...
	if _G[frame:GetName().."Text"] then
		local skinObj = _G[frame:GetName().."Text"]
		if skinObj then skinObj:ClearAllPoints() end
...
function S:HandleIconSelectionFrame(frame, numIcons, buttonNameTemplate, frameNameOverride)
	...
	--numIcons comes from constants like NUM_MACRO_ICONS_SHOWN, which do not
	--necessarily match how many buttons this client actually created
	for i = 1, numIcons do
