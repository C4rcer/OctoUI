...
local getn, setn, tremove, tContains, tinsert, wipe = table.getn, table.setn, tremove, tContains, tinsert, table.wipe
...
E.PopupDialogs["ELVUI_UPDATE_AVAILABLE"] = {
	...
	OnShow = function()
		...
		this.editBox:SetText("https://github.com/C4rcer/OctoUI")
	...
	EditBoxOnTextChanged = function()
		if this:GetText() ~= "https://github.com/C4rcer/OctoUI" then
			this:SetText("https://github.com/C4rcer/OctoUI")
		end
		this:HighlightText()
		this:ClearFocus()
	end,
...
E.PopupDialogs["TUKUI_ELVUI_INCOMPATIBLE"] = {
	...
	OnAccept = function() DisableAddOn("OctoUI") ReloadUI() end,
...
E.PopupDialogs["RESET_UF_UNIT"] = {
	...
	OnAccept = function()
		if this.data and this.data.unit then
			...
			if E.ConfigLoaded then
				local ACD = LibStub and LibStub("AceConfigDialog-3.0")
				if ACD and ACD.OpenFrames and ACD.OpenFrames[E.ConfigAppName] then
					ACD:SelectGroup(E.ConfigAppName, "unitframe", this.data.unit)
				end
			end
		else
...
function E:StaticPopup_OnShow()
	...
	-- boost static popups over ace gui: the config window is FULLSCREEN_DIALOG
	-- strata, so at the popups' native DIALOG strata the reload-confirm ends
	-- up buried under the panel that spawned it
	if E.ConfigLoaded then
...
function E:StaticPopup_OnKeyDown(key)
	if GetBindingKey(key) == "TOGGLEGAMEMENU" then
		return E:StaticPopup_EscapePressed()
	elseif GetBindingKey(key) == "SCREENSHOT" then
...
function E:StaticPopup_OnEvent()
	--1.12 event handlers receive no arguments; the frame arrives in the
	--global `this`. Every sibling handler here was already converted, this
	--one was missed because it only fires on display-size/ui-scale changes.
	this.maxHeightSoFar = 0
	E:StaticPopup_Resize(this, this.which)
end
...
function E:StaticPopup_Show(which, text_arg1, text_arg2, data)
	...
	--This client's Turtle-patched StaticPopupTemplate carries children this
	--ported file predates, which start out visible and sit on top of the
	--buttons. No ElvUI dialog uses them, so hide whichever exist.
	local moneyInput = _G[name.."MoneyInputFrame"]
	if moneyInput and not info.hasMoneyInputFrame then
		moneyInput:Hide()
	end
	local itemFrame = _G[name.."ItemFrame"]
	if itemFrame and not info.hasItemFrame then
		itemFrame:Hide()
	end
...
-- Set the miscellaneous variables for the dialog
	...
	do --If there is any recursion in this block, we may get errors (tempButtonLocs is static). If you have to recurse, we'll have to create a new table each time.
		assert(getn(tempButtonLocs) == 0) --If this fails, we're recursing. (See the table.wipe at the end of the block)
