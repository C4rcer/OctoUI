...
local function GetPoint(obj)
	...
	-- Only anchor was guarded here. A frame with no point set at all returns five
	-- nils, which reached E:Round and died on the arithmetic, and a nameless anchor
	-- put a literal "nil" into the string for SetPoint to choke on later. Movers are
	-- parented to ElvUIParent regardless, so fall back to it rather than erroring.
	if not anchor then anchor = ElvUIParent end
	local anchorName = anchor:GetName()
	if not anchorName then anchorName = "ElvUIParent" end
	if not point then point = "CENTER" end
	if not secondaryPoint then secondaryPoint = point end
	if not x then x = 0 end
	if not y then y = 0 end

	return format("%s,%s,%s,%d,%d", point, anchorName, secondaryPoint, E:Round(x), E:Round(y))
end
...
--Mover positions arrive as strings, having gone through GetPoint -> format -> split.
--Anything malformed in there reaches SetPoint, which throws its Usage error, and that
--aborts whatever was building the mover. CreateChatPanels is the painful case: it calls
--CreateMover for LeftChatMover at its very start, so a single bad mover meant
--RightChatPanel further down was never created at all, and everything referencing it
--then failed too. Resolve the anchor to a real frame rather than trusting a name to
--still resolve globally, and fall back to something usable instead of erroring.
local function SanitisePoint(point, anchor, secondaryPoint, x, y)
	if not validPoints[point] then point = "CENTER" end
	if not validPoints[secondaryPoint] then secondaryPoint = point end

	if type(anchor) == "string" then anchor = _G[anchor] end
	if not anchor then anchor = E.UIParent end

	return point, anchor, secondaryPoint, tonumber(x) or 0, tonumber(y) or 0
end
...
local function CreateMover(parent, name, text, overlay, snapOffset, postdrag, shouldDisable)
	...
	local point, anchor, secondaryPoint, x, y = SanitisePoint(split(",", GetPoint(parent)))
	...
	if E.db.movers and E.db.movers[name] then
		...
		--delim stays nil for a saved string containing neither delimiter, which would
		--have thrown from split itself. Fall back to the position read off the parent.
		if delim then
			local point1, anchor1, secondaryPoint1, x1, y1 = SanitisePoint(split(delim, anchorString))
