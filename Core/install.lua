...
function E:SetupLayout(layout, noDataReset)
	...
	if not noDataReset then
		...
		--The per-layout blocks below place bars 1, 2, 3 and 5 but never bar 4,
		--the pet action bar (ElvBar_Pet) or the stance bar (ShiftAB), so those
		--three kept their code defaults and sat in the screen corners: bar 4 in
		--particular ships as a 1-per-row vertical strip pinned to the right
		--edge. Set here, outside any layout branch, so every layout gets them.
		--Stacked upward from the main row: bar 4, then stance, then pet bar,
		--all centred. Stance and pet hide themselves when unused (no forms, no
		--pet), so a class with neither loses no space.
		E.db.actionbar.bar4.buttonsPerRow = 12
		E.db.movers.ElvAB_4 = "BOTTOM,ElvUIParent,BOTTOM,0,"..(E.PixelMode and 46 or 48)
		E.db.movers.ShiftAB = "BOTTOM,ElvUIParent,BOTTOM,0,"..(E.PixelMode and 88 or 92)
		E.db.movers.ElvBar_Pet = "BOTTOM,ElvUIParent,BOTTOM,0,"..(E.PixelMode and 130 or 136)
...
function E:Install()
	if not InstallStepComplete then
		...
		imsg.bg:SetTexture([[Interface\AddOns\OctoUI\media\textures\LevelUpTex]])
		...
		imsg.lineTop:SetTexture([[Interface\AddOns\OctoUI\media\textures\LevelUpTex]])
		...
		imsg.lineBottom:SetTexture([[Interface\AddOns\OctoUI\media\textures\LevelUpTex]])
	...
	if not ElvUIInstallFrame then
		...
		f.tutorialImage:SetTexture("Interface\\AddOns\\OctoUI\\media\\textures\\logo")
