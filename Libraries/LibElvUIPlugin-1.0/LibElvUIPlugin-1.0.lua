...
function lib:RegisterPlugin(name, callback, isLib)
	...
	--The real precondition for a plugin's option callback is that the engine
	--table and its options exist, not that some addon is "loaded". This library
	--is listed before Init.lua in the TOC, and IsAddOnLoaded("OctoUI") already
	--reports true while our own files are still running, so testing that made
	--the callback fire against a nil ElvUI. Test what is actually needed, and
	--otherwise defer to ADDON_LOADED, which fires once every file has run.
	local loaded = ElvUI and ElvUI[1] and ElvUI[1].ConfigLoaded
